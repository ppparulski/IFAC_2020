...
where 
$x = [q_{1}\ v_{1}\ q_{2}\ v_{2}]^\top$ is the state
and 
$x_r = [q_{r1}\ v_{r1}\ q_{r2}\ v_{r2}]^\top$ is the reference state, 
$\omega_0$ and $K = [k_1\ k_2\ k_3\ k_4]$ are controllers gains.
\\
%
As discussed above, the Pendubot control law consists of two regulators.
The first one is a swing-up-like type (Eq.~\ref{eq:RE_acrobot_u}), whose task is to bring the manipulator near the equilibrium point.
When the robot state is in the error tunnel near the equilibrium it switches to the linear controller (Eq.~\ref{eq:sterownik_liniowy}).
%
...
The aim of presented simulations is to stabilize the robot in upright position.
...
To check how the algorithm behaves near the equilibirum pose, the system  (\ref{eq:dynamika_f_x_g})  with only linear controller (\ref{eq:sterownik_liniowy}) working is checked.
...
\end{figure}
% 
{\color{black}To determine which controller should be used during stabilization process, a following switching procedure was implemented.
...
and $\gamma_d$ is a desired value of parameter $\gamma$ from Eq~(\ref{eq:wektory_f_oraz_g}).
}
%
...
%
The results in Fig.~\ref{fig:obszar_zbierznosci_stero_hybrid_z_saturacja} are for the case were motor torque is restricted to $6$~Nm, as in the section  \ref{sec:parametry_robota}.
%
