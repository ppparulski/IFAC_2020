...
To check {\color{red}properties of the algorithm near} the equilibrium pose, the system  (\ref{eq:dynamika_f_x_g})  with only linear controller (\ref{eq:sterownik_liniowy}) working is checked.
...
\end{figure}
%
The plot of angular positions in links, ground reaction forces, {\color{red}the determinant of the decoupling matrix (term $M_\delta = L_g L_f^2 h$)} and motor torque for exemplary successful trial are depicted in Fig.~(\ref{fig:LQR_pendu_th1th2_reakcja},~\ref{fig:LQR_pendu_mianownik_tau}).
...
\end{figure}
% 
{\color{black}To determine which controller should be used during stabilization process, a following switching procedure was implemented. One can present this procedure as a pseudo-code described by Algorithm~\ref{code:alg_1}:
%
\begin{algorithm}
...
For the Case~I and Case~II the results are depicted in Fig.~\ref{fig:obszar_zbierznosci_stero_hybrid_bez_saturacji} and in in Fig.~\ref{fig:obszar_zbierznosci_stero_hybrid_z_saturacja}, respectively.  
%
...
%
%
Simulations {\color{red} were} conducted with a step size equal 10$^\circ$, within range of $\theta_{1_0}$ from $0$ to $180^\circ$ and
...
Authors assumed  $\gamma_d = 0.04$, which was obtained by trail end error.
\\
