...
\\
{\color{red}
Thus the control strategy is as follows.
Firstly, authors obtained a set of initial conditions for which the control~(\ref{eq:RE_acrobot_u}) for system~(\ref{eq:dynamika_f_x_g}) brings the robot near the equilibrium point.
%(Tutaj mo¿e rysunek jak siê uda go zrobiæ).
\\
Then the convergence area  for the linear controller in the form of (\ref{eq:sterownik_liniowy}) was determined
%
\begin{equation}
u_{lin} = -K (x_r - x),
...
\end{equation}
%
which stabilizes the robot in the equilibrium pose (Fig.~\ref{fig:obszar_zbierznosci_tylko_liniowy}).
%
\begin{figure}[!ht]
	\centering
...
\end{figure}
%
As both controllers have a different convergence area, one decided to combine them in order to increase this area, obtaining the control rule for Pendubot  in the following form
...
\\
tutaj jakieœ ³adne wzorki opisujace te prze³¹czenie...
\\
W wyniku otrzymano obszar zbierznoœci przedstwiony na Fig.~\ref{fig:obszar_zbierznosci_stero_hybrid_bez_saturacji}.
%
\begin{figure}[!ht]
	\centering
...
\begin{figure}[!ht]
	...
	\caption{Obszar zbieznosci dla sterownika hybrydowego -- z saturacj¹}
