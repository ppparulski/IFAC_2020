...
\title{A hybrid controller for the Pendubot based on linearisation methods} 
...
\address[Second]{Institute of Automation and Robotics, 
	Poznan University of Technology, Poznan, Poland 
	(e-mail: patryk.bartkowiak@put.poznan.pl)}
...
{\color{red}Taking into consideration the last terms in Eqs. \eqref{eq:dynamika_wzorek_G} and 
\eqref{eq:wzor_1}}, one can obtain a relationship describing the equilibrium condition. Specifically, one can obtain the following relation:
$
\cos(\theta_1 + \theta_2) = 0
$.
%
However, for $\tau_1=0$ the system (\ref{eq:wzor_1})  has four characteristic equilibrium points: 
...
\label{sec:respondek}
%
The paper suggests to use algorithm originally proposed by \cite{Respondek_2019} to control the Pendubot from any initial condition to an equilibrium pose. This method can be briefly characterize  as follows.
...
$
%
After introducing {\color{red}the following new state variables}
%
\begin{equation}
...
$,
%and 
and {\color{red}transforming \eqref{eq:respondek_forma_spong} into normal form, one obtains equivalent dynamics defined by}
%
%
...
\end{equation}
%
where {\color{red}$x = [q_1\ {v}_1\ q_2\ v_2]^\top$ is a new state},
%
\begin{equation}
f(x) =
\left[
...
{\color{red}Now in order to find control $u$ we look for such an output function $h$ that maximally linearises \eqref{eq:Respondek_pendubot_nowe}.}
...
%
{\color{red}Then the following stabilising feedback can be proposed}
%
\begin{equation}
...
\end{equation}
%
{\color{red}where $L_f h$, $L_f^2 h$ are Lie derivatives along vector field $f(x)$ and}
%
\begin{equation}
...
\end{equation}
%
{\color{red}with $\omega_0>0$ being a gain scaling factor.}
...
\end{equation}
%
which stabilizes the robot in the equilibrium pose should be determined. {\color{red}In \eqref{eq:sterownik_liniowy} terms $x_r = [q_{r1}\ v_{r1}\ q_{r2}\ v_{r2}]^\top$ and $K = [k_1\ k_2\ k_3\ k_4]$ stand for the reference state and the controller gains, respectively.}
...
{\color{black}As both controllers have a different convergence area, one can propose to combine them in order to increase this area, obtaining the control rule for Pendubot  in the following form}
...
\end{equation}
%
{\color{red}Consequently, the proposed control law for the Pendubot consists of two regulators.} The first one is a swing-up-like type, Eq.~\eqref{eq:RE_acrobot_u}, whose task is to bring the manipulator near the equilibrium point.
When the robot state is in the error tunnel near the equilibrium it switches to the linear controller, Eq.~\eqref{eq:sterownik_liniowy}.
%
...
{\color{red}The aim of presented simulations is to check the performance of the controller applied for the stabilisation of Pendubot in the upright position. Another purpose is to verify the convergence area for the closed-loop system,} which means that the set of initial conditions are going to be checked to see whether they are appropriate to stabilise the robot under certain boundary conditions.
%
...
Secondly, the motor torque saturation is imposed on the model (Case II). {\color{red}Taking into account properties of the real robot  the torque magnitude is restricted to $6$~Nm.}
Thus, the algorithm is being checked whether is capable of controlling the model of physical robot.
%

It is interesting to see how far can the Pendubot be moved away from the equilibrium pose, to be stabilized in the upright position under certain boundary conditions. The desired stabilisation pose was the upright position for which the angles $\theta_{1r}$ and $\theta_{2r}$ were equal $\frac{\pi}{2}$ and $0$, respectively.
%
To determine the convergence area of analysed method, a~series of tests were conducted.
To check {\color{red}properties of the algorithm near} the equilibrium pose, the system  (\ref{eq:dynamika_f_x_g})  with only linear controller (\ref{eq:sterownik_liniowy}) working is verified.
...
\end{figure}
%
The plot of angular positions in links, ground reaction forces, {\color{red}the determinant of the decoupling matrix (term $M_\delta = L_g L_f^2 h$)} and motor torque for exemplary successful trial are depicted in Figs.~(\ref{fig:LQR_pendu_th1th2_reakcja},~\ref{fig:LQR_pendu_mianownik_tau}).

Subsequently, the hybrid controller (\ref{eq:pendubot_u_hybrydowe}) was considered to verify {\color{red}the effectiveness of the proposed} control scheme.
%
\begin{figure}[!h]
	\centering
...
\end{figure}
% 
{\color{black}To determine which controller should be used during stabilization process, the following switching procedure was implemented. One can present this procedure as a pseudo-code described by Algorithm~\ref{code:alg_1}:
%
\begin{algorithm}
...
\end{algorithm}
%
where $a_1, a_2, b_1, b_2$ are coefficients of straight  lines $y_i = a_i \cdotp \theta_{j,0} + b_i$ for $i=\{1,2\}$ which stand for the lower and upper bound of a convergence area from Fig.~\ref{fig:obszar_zbierznosci_tylko_liniowy}, 
and $\gamma_d$ is \color{red} the chosen upper bound of $|\gamma|$} from Eq~\eqref{eq:wektory_f_oraz_g}.
}
%
...
%
The blue {\color{red}cell} indicates the successful trial, while the red one indicates the initial conditions that lead to failure (robot collapse).
%
%
%
\begin{figure}[!ht]
...
%
%
Simulations {\color{red} were} conducted with a step size equal to 10$^\circ$, within range of $\theta_{1_0}$ from $0$ to $180^\circ$ and
$\theta_{2_0}$ from $-90^\circ$ to $90^\circ$, which gave more than 1300 simulation per approach.
These initial conditions mean that the swing-up problem is not considered, i.e. only {\color{red} scenarios} where the robot is above the ground are taken into account.
The trial was being taken as successful while the error between actual and desired angle was smaller than  $\frac{1}{180}\pi \,$rad within simulation time  {\color{black} $t = 15 \,$s}. 
%
...
This situation is not favourable, as there is no any initial {\color{red}kinetic} energy in the system at the beginning of the Pendubot movement.
...
Moreover, the robot is not fixed to the ground and the reaction forces are considered preventing the robot to {\color{red}lift-off}.
%
{\color{red}The considered algorithm does not guarantee that trajectory $x(t)$ will not reach singularity.} Thus, other constrains had to be imposed on the model.
...
First of them is a "standard" simulation time, computed by a numerical solver.
The other one is the time of program execution (or iterations of the program, to become independent of the computer CPU).
The need of imposing such a constraint results from the nature of the considered system.
...
%
Authors concluded that while {\color{black} 50000 iterations} of the program execution no result was obtained (achieving the desired configuration), such a situation should be treated as the occurrence of singularities in the object and a crash of the program. 
\\
...
{\color{red}The main objective of this paper is to evaluate properties of the new control approach designed for the Pendubot. Based on the obtained results it is clear that the proposed hybrid controller makes it possible to overcome limitations of the algorithms discussed in \cite{Respondek_2019}, namely  the convergence set can be increased significantly. One can conclude that application of linearisation technique with respect to the Pendubot is challenging due to singular points. Hence, this approach can be seen rather as the representation of Pendubot properties than an overall recipe  for stabilizing a double inverted pendulum with one actuation. Basically, the algorithm does not guarantee that trajectories of the closed-loop system do not reach singularities. In order to overcome this issue one could employ tracking of feasible reference trajectory. This concept, however, will be explored in future works.}
