...
%		{$\frac{1}{L_g L_f^2 h} > 0.001$}
		{$\frac{1}{\left| L_g L_f^2 h\right|} > \delta$}
		\State $u = u_h$
		\Else
		\State $u=0$
		\EndIf
...
One can draw a conclusion that analysed approach is rather a representation of Pendubot's properties than an overall recipe  for stabilizing a double inverted pendulum with one actuation.
Basically, the algorithm does not guarantee that trajectories of the closed-loop system do not reach singularities. In order to overcome this issue one could employ tracking of feasible reference trajectory. This concept, however, will explored in future works.
